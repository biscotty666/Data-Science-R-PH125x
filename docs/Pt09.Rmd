...
<blockquote>
__Additional material from the on-line course__
...
- Standard units describe how many standard deviations a value is away from the mean. The z-score, or number of standard deviations an observation  is away from the mean $\mu$:

$$
Z=\frac{x-\mu}{\sigma}
$$
- Compute standard units with the `scale()` function.
- __Important__: to calculate the proportion of values that meet a certain condition, use the mean() function on a logical vector. Because TRUE is converted to 1 and FALSE is converted to 0, taking the mean of this vector yields the proportion of TRUE.
...
NOTE: The average and standard deviation obtained by using the code below are 69.31 and 3.61, respectively. These values appear in the video, but differ slightly from the audio.
...
<img src="https://courses.edx.org/assets/courseware/v1/28d10d22ba8b3bf4b0cc5024de0dc616/asset-v1:HarvardX+PH125.2x+3T2022+type@asset+block/norm-dist-probs-combined.png" />

The probability distribution function for the normal distribution is defined such that:

- About 68% of observations will be within one standard deviation of the mean ($\mu\pm\sigma$). In standard units, this is equivalent to a z-score of $|z|\le1$.
- About 95% of observations will be within two standard deviations of the mean ($\mu\pm2\sigma$). In standard units, this is equivalent to a z-score of $|z|\le2$.
- About 99.7% of observations will be within three standard deviations of the mean ($\mu\pm3\sigma$). In standard units, this is equivalent to a z-score of $|z|\le3$.
</blockquote>
<blockquote id="pnorm">
__`pnorm`__
...
- If we treat the height data as discrete rather than categorical, we see that the data are not very useful because integer values are more common than expected due to rounding. This is called __discretization__.
...
__Using pnorm to calculate probabilities__

We can estimate the probability that a male is at least 70.5 inches with:
```{r}
pnorm(70.5, mean(x), sd(x))
```

__plot distribution of exact heights in data__
```{r}
plot(prop.table(table(x)), xlab = "a = Height in inches", ylab = "Pr(x = a)")
```

__probabilities in actual data over length 1 ranges containing an integer__
```{r}
...
__probabilities in normal approximation match well__
```{r}
...
__probabilities in actual data over other ranges don't match normal approx as well__
```{r}
...
<blockquote id="quantile">
__Definition of quantiles__
...
__Using the quantile function__
...
$$
\text{pnorm}(-1.96)\approx0.025
$$
The result of pnorm() is the quantile. Note that:
$$
\text{qnorm}(0.025)\approx-1.96
$$
qnorm() and pnorm() are inverse functions:
$$
\text{pnorm}(\text{qnorm}(0.025))=0.025
$$
__Theoretical quantiles__
